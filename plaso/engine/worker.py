...
class EventExtractionWorkerVolumeScanner(dfvfs_volume_scanner.VolumeScanner):
    ...
    def GetBasePathSpecs(
        self, source_path_spec, options=None
...
class EventExtractionWorker:
    ...
    _TYPES_WITH_ROOT_METADATA = frozenset([dfvfs_definitions.TYPE_INDICATOR_GZIP])
    ...
    def _AnalyzeFileObject(self, file_object, display_name, event_data_stream):
        ...
        while data:
            ...
            for analyzer_object in self._analyzers:
                ...
                if (
                    not analyzer_object.INCREMENTAL_ANALYZER
                    and file_size > analyzer_object.SIZE_LIMIT
                ):
                    continue

                if (
                    isinstance(analyzer_object, hashing_analyzer.HashingAnalyzer)
                    and self._hasher_file_size_limit
                    and file_size > self._hasher_file_size_limit
                ):
                    continue
        ...
        for analyzer_object in self._analyzers:
            for result in analyzer_object.GetResults():
                ...
                setattr(
                    event_data_stream, result.attribute_name, result.attribute_value
                )
    ...
    def _CanSkipContentExtraction(self, file_entry):
        ...
        if self._CHROME_CACHE_DATA_FILE_RE.match(path_segments[-1]):
            ...
            len(path_segments) == 4
            and path_segments[0].lower() == "private"
            and path_segments[1].lower() == "var"
            and path_segments[2].lower() == "vm"
            and path_segments[3].lower() in ("sleepimage", "swapfile0", "swapfile1")
        ):
            return True
    ...
    def _CreateArchiveTypeScanner(self, archive_types):
        ...
        if "modi" in archive_types:
            ...
            scanner_object.add_signature(
                "modi_udif", 512, b"koly", pysigscan.signature_flags.RELATIVE_FROM_END
            )

        if "tar" in archive_types:
            scanner_object.add_signature(
                "tar", 257, b"ustar\x00", pysigscan.signature_flags.RELATIVE_FROM_START
            )
            scanner_object.add_signature(
                "tar_old",
                257,
    ...
    def _ExtractMetadataFromFileEntry(self, parser_mediator, file_entry, data_stream):
        ...
        self._event_data_extractor.ParseFileEntryMetadata(parser_mediator, file_entry)
    ...
    def _IsMetadataFile(self, file_entry):
        ...
        if (
            ...
            and file_entry.path_spec.location in self._METADATA_FILE_LOCATIONS_NTFS
        ):
            return True

        if (
            file_entry.type_indicator == dfvfs_definitions.TYPE_INDICATOR_TSK
            and file_entry.path_spec.location in self._METADATA_FILE_LOCATIONS_TSK
        ):
            return True
    ...
    def _ProcessStorageMediaImageType(self, parser_mediator, path_spec, type_indicator):
    ...
    def _ShouldSkipFileEntry(self, parser_mediator, file_entry):
        ...
        """
        if not (self._max_file_size or self._file_date_start or self._file_date_end):
            return False
            ...
            try:
                ...
                for timestamp in (
                    ...
                    if timestamp is None:
                        continue
                    if (
                        self._file_date_start is not None
                        and timestamp < self._file_date_start
                    ):
                        continue
                    if (
                        self._file_date_end is not None
                        and timestamp > self._file_date_end
                    ):
                        continue
                    in_range = True
                    break
    ...
    def _ProcessFileEntry(self, parser_mediator, file_entry):
        ...
        """
        display_name = parser_mediator.GetDisplayName()
        logger.debug(f"[ProcessFileEntry] processing file entry: {display_name:s}")
        ...
        logger.debug(f"[ProcessFileEntry] done processing file entry: {display_name:s}")

    def _ProcessFileEntryDataStream(self, parser_mediator, file_entry, data_stream):
        ...
        if compressed_stream_types:
            ...
            if results == ["iso9660"]:
                ...
                if len(results) > 1:
                    display_name = parser_mediator.GetDisplayName()
                    logger.debug(
                        f"Found multiple format type indicators: {results!s} for "
                        f"archive file: {display_name:s}"
                    )
