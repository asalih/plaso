...
class EventDataTimeliner:
        ...
        # in the SQLite storage.
            ...
            if date_time.is_local_time:
                ...
                if not time_zone and event_data_stream:
                    try:
                        time_zone = self._GetTimeZoneByPathSpec(
                            event_data_stream.path_spec
                        )
                    except pytz.UnknownTimeZoneError:
                        message = (
                            f"unsupported system time zone: "
                            f"{date_time.time_zone_hint:s}, using default time zone"
                        )
                        self._ProduceTimeliningWarning(
                            storage_writer,
    ...
    def _ReadConfigurationFile(self):
        ...
        Raises:
          KeyError: if the attribute mappings are already set for the corresponding
              data type.
        """
        path = os.path.join(self._data_location, self._TIMELINER_CONFIGURATION_FILENAME)
    ...
    def ProcessEventData(self, storage_writer, event_data, event_data_stream):
        ...
        attribute_mappings = self._attribute_mappings.get(event_data.data_type) or {}
        if (
            not attribute_mappings
            and event_data.data_type not in self._place_holder_event
        ):
            return
    ...
    def _ProcessEventDataConsolidated(
        ...
        """Generates a single consolidated event from event data.
        ...
        # If no timestamp was found, use a placeholder.
        ...
        try:
            ...
            warning_message = f"unable to add event with error: {exception!s}"
            self._ProduceTimeliningWarning(storage_writer, event_data, warning_message)
