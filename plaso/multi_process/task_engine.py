...
class TaskMultiProcessEngine(engine.MultiProcessEngine):
    ...
    * manage task storage used to store task results.
    ...
    def _RunFileOperationWithRetry(self, operation, description):
        ...
        for attempt in range(self._WINDOWS_FILE_OPERATION_RETRY_ATTEMPTS):
            try:
                ...
                is_windows_sharing_error = getattr(
                    exception, "winerror", None
                ) == 32 or (
                    os.name == "nt" and exception.errno in (errno.EACCES, errno.EBUSY)
                )
                if (
                    not is_windows_sharing_error
                    or attempt + 1 == self._WINDOWS_FILE_OPERATION_RETRY_ATTEMPTS
                ):
                    raise

                logger.warning(
                    f"Unable to {description:s} due to a Windows sharing "
                    f"violation; retrying attempt {attempt + 2:d} of "
    ...
    def _GetMergeTaskStorage(self, task_storage_format, task):
        ...
        task_storage_reader = storage_factory.StorageFactory.CreateTaskStorageReader(
    ...
    def _GetProcessedTaskIdentifiers(self, task_storage_format, session_identifier):
    ...
    def _PrepareMergeTaskStorage(self, task_storage_format, task):
        ...
        if task_storage_format == definitions.STORAGE_FORMAT_REDIS:
            ...
            self._redis_client.hset(redis_hash_name, key=task.identifier, value=b"true")
    ...
    def _StartTaskStorage(self, task_storage_format):
        ...
        if task_storage_format == definitions.STORAGE_FORMAT_REDIS and redis_store:
            ...
            # Handle case where storage_file_path is None (e.g. in JSON stdout
            # mode) by falling back to the system temporary directory.
    ...
    def _StopTaskStorage(self, task_storage_format, session_identifier, abort=False):
            ...
            if os.path.isdir(self._merge_task_storage_path):
                if abort:
                    self._RunFileOperationWithRetry(
                        ...
                        f"remove merge task storage: "
                        f"{self._merge_task_storage_path:s}",
                    )
                else:
                    self._RunFileOperationWithRetry(
