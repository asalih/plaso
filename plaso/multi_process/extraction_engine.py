    ...
    * merge results returned by extraction worker processes.
        ...
        while not self._abort:
            ...
            if fully_merged:
                ...
                try:
                    ...
                    logger.error(
                        f"Unable to complete task: {self._merge_task.identifier:s} "
    ...
    def _ProcessEventSources(self, storage_writer, session_identifier):
            ...
            try:
                ...
                if task:
                    if not self._ScheduleTask(task):
                        ...
                        logger.debug(
                            f"Scheduled task: {task.identifier:s} for path "
                            f"specification: {path_spec_string:s}"
                        )
        ...
        for task in self._task_manager.GetFailedTasks():
            self._ProduceExtractionWarning(
                storage_writer,
                "Worker failed to process path specification",
                task.path_spec,
            )
    ...
    def _UpdateProcessingStatus(self, pid, process_status, used_memory):
        ...
        number_of_skipped_sources = process_status.get("number_of_skipped_sources", 0)
        previous_number_of_skipped_sources = self._number_of_skipped_sources_per_pid.get(
            pid, 0
        )
    ...
    def ProcessSourceMulti(
        ...
        self._event_data_timeliner = timeliner.EventDataTimeliner(
            ...
            consolidated_timestamps=(
                processing_configuration.consolidated_timestamps
            ),
        )
        try:
        ...
        try:
            self._StopTaskStorage(
                self._task_storage_format, session_identifier, abort=task_storage_abort
            )
        except OSError as exception:
