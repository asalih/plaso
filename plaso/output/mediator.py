...
class OutputMediator:
    ...
    def _ReadMessageFormattersFile(self, path, override_existing=False):
        ...
        for message_formatter in message_formatters_file.ReadFromFile(path):
            if (
                message_formatter.data_type in self._message_formatters
    ...
    def _GetSourcePath(self):
        ...
        if self._source_path is None:
            source_configurations = list(
                self._storage_reader.GetAttributeContainers("source_configuration")
            )
        ...
        if self._relative_paths and display_name:
            ...
            if source_path:
                ...
                if path_part.startswith(source_path):
                    ...
                    # If the path is empty after stripping, use '.' to represent
    ...
    def GetMACBRepresentation(self, event, event_data):
        ...
        if event.timestamp_desc == (definitions.TIME_DESCRIPTION_METADATA_MODIFICATION):
            return "..C."
    ...
    def GetMACBRepresentationFromDescriptions(self, timestamp_descriptions):
        ...
        if (
            "mtime" in timestamp_descriptions
        ...
        if (
            "atime" in timestamp_descriptions
        ...
        if (
            "ctime" in timestamp_descriptions
            or definitions.TIME_DESCRIPTION_METADATA_MODIFICATION
        ...
        if (
            "crtime" in timestamp_descriptions
    ...
    def GetRelativePathForPathSpec(self, path_spec):
        ...
        if self._relative_paths and relative_path:
            ...
            if source_path and relative_path.startswith(source_path):
                ...
                if relative_path.startswith("/") or relative_path.startswith("\\"):
                    relative_path = relative_path[1:]
                # If the path is empty after stripping, use '.' to represent the
                # current directory.
                if not relative_path:
                    relative_path = "."
