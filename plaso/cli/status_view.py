...
class StatusView:
    ...
    _SOURCE_TYPES = {
        ...
        dfvfs_definitions.SOURCE_TYPE_STORAGE_MEDIA_DEVICE: ("storage media device"),
        dfvfs_definitions.SOURCE_TYPE_STORAGE_MEDIA_IMAGE: ("storage media image"),
    }
    ...
    _WINAPI_ANSI_CONSOLE_MODE = (
        _WINAPI_ENABLE_PROCESSED_INPUT
        | _WINAPI_ENABLE_LINE_INPUT
    ...
    def __init__(self, output_writer, tool_name):
        ...
        self._stdout_output_writer = isinstance(output_writer, tools.StdoutOutputWriter)
    ...
    def _AddsAnalysisProcessStatusTableRow(self, process_status, table_view):
        ...
        events = ""
        if (
            process_status.number_of_consumed_events is not None
            and process_status.number_of_consumed_events_delta is not None
        ):
            events = (
        ...
        event_tags = ""
        if (
            process_status.number_of_produced_event_tags is not None
            and process_status.number_of_produced_event_tags_delta is not None
        ):
            event_tags = (
        ...
        reports = ""
        if (
            process_status.number_of_produced_reports is not None
            and process_status.number_of_produced_reports_delta is not None
        ):
            reports = (
    ...
    def _AddExtractionProcessStatusTableRow(
        ...
        sources = ""
        if (
            process_status.number_of_produced_sources is not None
            and process_status.number_of_produced_sources_delta is not None
        ):
            sources = (
        ...
        if show_events:
            if (
                process_status.number_of_produced_events is not None
                and process_status.number_of_produced_events_delta is not None
            ):
                events_or_event_data = (
            ...
            if (
                process_status.number_of_produced_event_data is not None
                and process_status.number_of_produced_event_data_delta is not None
            ):
                events_or_event_data = (
    ...
    def _PrintAnalysisStatusHeader(self, processing_status):
        ...
        Args:
          processing_status (ProcessingStatus): processing status.
        """
        self._output_writer.Write(f"Storage file\t\t: {self._storage_file_path:s}\n")
    ...
    def _PrintExtractionStatusUpdateJSON(self, processing_status):
        ...
        status = {
            ...
            "tasks": {
                "abandoned": getattr(tasks_status, "number_of_abandoned_tasks", 0),
                "pending_merge": getattr(
                    tasks_status, "number_of_tasks_pending_merge", 0
                ),
                "processing": getattr(tasks_status, "number_of_tasks_processing", 0),
                "queued": getattr(tasks_status, "number_of_queued_tasks", 0),
                "skipped": getattr(tasks_status, "number_of_skipped_sources", 0),
    ...
    def PrintExtractionSummary(self, processing_status, number_of_extraction_warnings):
