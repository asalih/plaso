...
class StatusViewArgumentsHelper(interface.ArgumentsHelper):
    ...
    def AddArguments(cls, argument_group):
        ...
        argument_group.add_argument(
            "--status_view",
            "--status-view",
            dest="status_view_mode",
            choices=cls._STATUS_VIEW_TYPES,
            action="store",
            metavar="TYPE",
            default=status_view.StatusView.MODE_WINDOW,
            help=(
        ...
        argument_group.add_argument(
            "--status_view_file",
            "--status-view-file",
            dest="status_view_file",
            action="store",
            metavar="PATH",
            default="status.info",
            help=("The name of the status view file."),
        )

        argument_group.add_argument(
            "--status_view_interval",
            "--status-view-interval",
            dest="status_view_interval",
            action="store",
            type=float,
            metavar="SECONDS",
            default=0.5,
            help=("Number of seconds to update the status view."),
        )
    ...
    def ParseOptions(cls, options, configuration_object):
        ...
        status_view_interval = cls._ParseNumericOption(options, "status_view_interval")
