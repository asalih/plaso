...
class ExtractionTool(
    ...
    def _ParseProcessingOptions(self, options):
        ...
        if (
            self._file_date_start
            and self._file_date_end
            and self._file_date_start > self._file_date_end
        ):
            raise errors.BadConfigOption(
    ...
    def ExtractEventsFromSources(self):
        ...
        if json_stdout_mode:
            ...
            try:
                ...
                raise OSError(f"Unable to open HTTP storage with error: {exception!s}")
        else:
