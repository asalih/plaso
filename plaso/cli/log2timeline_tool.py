...
class Log2TimelineTool(extraction_tool.ExtractionTool):
    ...
    def ParseArguments(self, arguments):
        ...
        info_group.add_argument(
            "--no-stream-events-storage",
            dest="no_stream_events_storage",
            action="store_true",
            default=False,
            help=(
        ...
        info_group.add_argument(
            ...
            help=(
                ...
                "Example: \"date > '2024-09-15 08:00:00' and date < "
                "'2024-09-15 18:00:00'\""
            ),
        )
    ...
    def ParseOptions(self, options):
        ...
        if self.stream_storage != "sqlite" or not self.store_events_in_storage:
            if not (self.json_stdout or self.http_endpoint):
                raise errors.BadConfigOption(
                    "--stream-storage and --no-stream-events-storage can only be "
                    "used with --json-stdout or --http-endpoint."
                )
        ...
        if event_filter_expression:
            if not (self.json_stdout or self.http_endpoint):
                raise errors.BadConfigOption(
                    "--event-filter can only be used with --json-stdout or "
                    "--http-endpoint."
                )
            ...
            try:
                ...
                raise errors.BadConfigOption(
                    f"Unable to compile event filter expression with error: "
                    f"{exception!s}"
                )
        else:
