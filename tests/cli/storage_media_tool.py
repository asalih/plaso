...
class StorageMediaToolVolumeScannerTest(test_lib.CLIToolTestCase):
    ...
    def _TestScanSourceAPFSImage(self, source_path):
        ...
        base_path_specs = []
        scan_context = test_scanner.ScanSource(source_path, options, base_path_specs)
        ...
        self.assertIsNotNone(scan_node)
        self.assertEqual(
            scan_node.type_indicator, dfvfs_definitions.TYPE_INDICATOR_APFS_CONTAINER
        )
        self.assertEqual(len(scan_node.sub_nodes), 1)
    ...
    def _TestScanSourceDirectory(self, source_path):
        ...
        scan_node = scan_context.GetRootScanNode()
        self.assertIsNotNone(scan_node)
        self.assertEqual(scan_node.type_indicator, dfvfs_definitions.TYPE_INDICATOR_OS)
    ...
    def _TestScanSourceLVMImage(self, source_path):
        ...
        scan_node = self._GetTestScanNode(scan_context)
        self.assertIsNotNone(scan_node)
        self.assertEqual(scan_node.type_indicator, dfvfs_definitions.TYPE_INDICATOR_LVM)
    ...
    def _TestScanSourcePartitionedImage(self, source_path):
        ...
        self.assertIsNotNone(scan_node)
        self.assertEqual(
            scan_node.type_indicator, dfvfs_definitions.TYPE_INDICATOR_TSK_PARTITION
        )
    ...
    def _TestScanSourceVSSImage(self, source_path):
        ...
        self.assertIsNotNone(scan_node)
        self.assertEqual(
            scan_node.type_indicator, dfvfs_definitions.TYPE_INDICATOR_QCOW
        )
        self.assertEqual(len(scan_node.sub_nodes), 2)
...
{test_lib.ARGPARSE_OPTIONS}:
  --credential TYPE:DATA
                        ...
                        recovery_password, startup_key, skip. Binary key data
                        is expected to be passed in BASE-16 encoding
